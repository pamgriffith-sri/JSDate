...
Date.prototype.format = function (formatstr) {
	...
	var formats = {
		...
		, W: function () {
			var jan_first = new Date(dateObj.getFullYear(),0,1);
			var jan_first_day = jan_first.format('N') - 1;
			var current_day_num = formats.z() + 1;
			var week_num = Math.floor((current_day_num + jan_first_day - 1) / 7);
			//if the year starts before the middle of a week
			if(jan_first_day < 4) {
				week_num += 1;
			} else if (week_num == 0) {
				// the first week is part of the last week of the previous year
				var prev_week = new Date(dateObj.getFullYear() - 1, 11, 31);
				week_num = parseInt(prev_week.format('W'));
			}
			if(week_num > 52) {
				next_year = new Date(dateObj.getFullYear() + 1,0,1);
				next_year_day = next_year.format('N') - 1;
				/*if the next year starts before the middle of
				the week, it is week #1 of that year*/
				week_num = next_year_day < 4 ? 1 : 53;
			}
			return util.zerofill(week_num, 2);
		}
